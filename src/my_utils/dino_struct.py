...
class DinoStructureLoss:
    ...
    def calculate_global_ssim_loss(self, outputs, inputs):
        ...
        for a, b in zip(inputs, outputs):  # avoid memory limitations
            with torch.no_grad():
                target_keys_self_sim = self.extractor.get_keys_self_sim_from_input(a.unsqueeze(0).to(self.device),
                                                                                  layer_num=11)
            keys_ssim = self.extractor.get_keys_self_sim_from_input(b.unsqueeze(0).to(self.device), layer_num=11)
