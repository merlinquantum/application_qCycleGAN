...
from my_utils.device_utils import get_default_device


def convert_lora_to_base(state_dict):
    new_state_dict = {}
    count = 0
    for key, value in state_dict.items():

        if "conv_in" in key or "conv_out" in key:
            count += 1
            # Skip LoRA layers
            if ".lora_" in key:
                continue
...
def initialize_unet(rank, return_lora_module_names=False):
    logger.info('[Init] Loading UNet from "stabilityai/sd-turbo" (subfolder="unet")')
    unet = UNet2DConditionModel.from_pretrained("stabilityai/sd-turbo", subfolder="unet")
    num_params = sum(p.numel() for p in unet.parameters())
    logger.info("[ModelFactory] UNet parameter count: {}", num_params)
...
def initialize_vae(rank=4, return_lora_module_names=False, dynamic = False):
    logger.info('[Init] Loading VAE from "stabilityai/sd-turbo" (subfolder="vae")')
...
class CycleGAN_Turbo(torch.nn.Module):
    def __init__(self, pretrained_name=None, pretrained_path=None, ckpt_folder="checkpoints", lora_rank_unet=8,
                 lora_rank_vae=4, accelerator=None, device=None):
        super().__init__()
        ...
        logger.info('[Init] Loading base components from "stabilityai/sd-turbo" (tokenizer/text_encoder/vae/unet)')
        ...
        self.sched = make_1step_sched(device=self.device)
        self.vae = AutoencoderKL.from_pretrained("stabilityai/sd-turbo", subfolder="vae").to(self.device)
        self.unet = UNet2DConditionModel.from_pretrained("stabilityai/sd-turbo", subfolder="unet").to(self.device)
        ...
        # add the skip connection convs
        self.vae.decoder.skip_conv_1 = torch.nn.Conv2d(512, 512, kernel_size=(1, 1), stride=(1, 1), bias=False).to(self.device)
        self.vae.decoder.skip_conv_2 = torch.nn.Conv2d(256, 512, kernel_size=(1, 1), stride=(1, 1), bias=False).to(self.device)
        self.vae.decoder.skip_conv_3 = torch.nn.Conv2d(128, 512, kernel_size=(1, 1), stride=(1, 1), bias=False).to(self.device)
        self.vae.decoder.skip_conv_4 = torch.nn.Conv2d(128, 256, kernel_size=(1, 1), stride=(1, 1), bias=False).to(self.device)
        ...
        if pretrained_name == "day_to_night":
            ...
            logger.info(f"[Checkpoint] Loading from provided pretrained path: {pretrained_path}")
    ...
    def load_ckpt_from_state_dict(self, sd, dyn=False):
        logger.info("[Checkpoint] Loading checkpoint state dict")
        ...
        #self.vae_b2a = copy.deepcopy(self.vae)
        #self.vae_enc = VAE_encode(self.vae, vae_b2a=self.vae_b2a)
        sd_vae_enc = self._prepare_vae_state(sd["sd_vae_enc"])
        self.vae_enc.load_state_dict(sd_vae_enc, strict=False)
        sd_vae_dec = self._prepare_vae_state(sd["sd_vae_dec"])
        self.vae_dec.load_state_dict(sd_vae_dec, strict=False)

    @staticmethod
    def _prepare_vae_state(state_dict):
        """Normalize VAE checkpoints supporting both LoRA and plain conv weights."""
        sample_key = next(iter(state_dict), "")
        if ".base_layer" in sample_key or ".lora_" in sample_key:
            return state_dict
        remapped = {}
        for key, value in state_dict.items():
            new_key = key
            if ".conv_in." in key and ".base_layer." not in key:
                new_key = key.replace(".conv_in.", ".conv_in.base_layer.")
            elif ".conv_out." in key and ".base_layer." not in key:
                new_key = key.replace(".conv_out.", ".conv_out.base_layer.")
    ...
    def forward_with_networks(x, direction, vae_enc, unet, vae_dec, sched, timesteps, text_emb):
        ...
        x_out = torch.stack(
            [sched.step(model_pred[i], timesteps[i], x_enc[i], return_dict=True).prev_sample for i in range(B)])
        x_out_decoded = vae_dec(x_out, direction=direction)
        return x_out_decoded


    @staticmethod
    def forward_with_networks_dynamic(x, direction, vae_enc, unet, vae_dec, sched, timesteps, text_emb, bs=None,
                                      q_emb=None,device = None, accelerator = None):

        B = x.shape[0]
        starter_bool = q_emb is None #will return q_emb if none was given as input
        assert direction in ["a2b", "b2a"]
        if bs is None and q_emb is None:
            raise AttributeError("Cannot compute quantum embeddings if boson sampler not given")

        # monitor NaN occurence
        if torch.isnan(x).any():
            logger.warning("[Trainer] NaN detected at VAE encoder input")
        ...
        if torch.isnan(x_enc).any():
            logger.warning("[Trainer] NaN detected at VAE encoder output")
        if q_emb is None:
            # need to compute the quantum embedding from the VAE encoder

            q_emb = bs(torch.sigmoid(x_enc))

            if torch.isnan(q_emb).any():
                logger.warning("[Trainer] Replacing NaN embeddings with random values")
    ...
    def get_traininable_params(unet, vae_a2b, vae_b2a, boson_sampler=None, dynamic=False, train_full_unet=False):
        def _maybe_base_params(module: torch.nn.Module):
            if module is None:
                return []
            base = getattr(module, "base_layer", None)
            return [p for p in (base or module).parameters() if p.requires_grad]

        def _extend_unique(params, new_params):
        ...
        # Always include VAE decoder IO conv base weights when they are trainable.
        _extend_unique(params_gen, _maybe_base_params(getattr(vae_a2b.decoder, "conv_in", None)))
        _extend_unique(params_gen, _maybe_base_params(getattr(vae_a2b.decoder, "conv_out", None)))
        _extend_unique(params_gen, _maybe_base_params(getattr(vae_b2a.decoder, "conv_in", None)))
        _extend_unique(params_gen, _maybe_base_params(getattr(vae_b2a.decoder, "conv_out", None)))

        # Include post_quant_conv if trainable (used by both classical-comparison and quantum runs).
        _extend_unique(params_gen, _trainable_params(getattr(vae_a2b, "post_quant_conv", None)))
        _extend_unique(params_gen, _trainable_params(getattr(vae_b2a, "post_quant_conv", None)))

        if not train_full_unet:
            for n, p in unet.named_parameters():
                if "lora" in n and "default" in n:
                    if p.requires_grad:
                        _extend_unique(params_gen, [p])

        # add all vae_a2b parameters
        for n, p in vae_a2b.named_parameters():
            if "lora" in n and "vae_skip" in n:
                if p.requires_grad:  # Only add if trainable (encoder may be frozen)
                    _extend_unique(params_gen, [p])

        _extend_unique(params_gen, list(vae_a2b.decoder.skip_conv_1.parameters()))
        _extend_unique(params_gen, list(vae_a2b.decoder.skip_conv_2.parameters()))
        _extend_unique(params_gen, list(vae_a2b.decoder.skip_conv_3.parameters()))
        _extend_unique(params_gen, list(vae_a2b.decoder.skip_conv_4.parameters()))
        ...
        _extend_unique(params_gen, list(vae_b2a.decoder.skip_conv_1.parameters()))
        _extend_unique(params_gen, list(vae_b2a.decoder.skip_conv_2.parameters()))
        _extend_unique(params_gen, list(vae_b2a.decoder.skip_conv_3.parameters()))
        _extend_unique(params_gen, list(vae_b2a.decoder.skip_conv_4.parameters()))
        if boson_sampler is not None:
            _extend_unique(params_gen, list(boson_sampler.parameters()))
