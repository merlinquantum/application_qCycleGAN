...
class BosonSampler:
    ...
    def __init__(self, dims : list|tuple, eps : float = 1e-5, circuit : None|pcvl.Circuit = None, trainable_parameters : list = None) -> None:
        ...
        print(f"[Quantum] Time to load model: {time.time() - start:.3f}s")
        ...
        for state in product(*ranges): # (range(self.dims[0], ..., range(self.dims[-1], range(2))
            ...
            if state[-1]==0:
                ...
                if fock_state_tuple in all_outputs_idx_map:
                    ...
                    print(f"[Quantum] Warning: State {fock_state_tuple} not found in output space")
            index += 1
...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        print(f"[Quantum Training] Iteration {i+1}, loss: {loss.item():.6f}")
        optimizer.step()
        optimizer.zero_grad()
